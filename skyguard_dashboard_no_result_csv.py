...
if (
    ...
    historical_top = historical_summary.get("top_anomalies")

    if (
        historical_top is not None
        and not historical_top.empty
    ):
        st.subheader(
            "🚨 Highest-Scoring Historical Observations"
        )

        top_display = historical_top.copy()

        # The existing summary uses "severity", while the live
        # dashboard uses "risk_level". Rename only for display.
        if "severity" in top_display.columns:
            top_display["risk_level"] = (
                top_display["severity"]
                .astype(str)
                .str.upper()
            )
        ...
        available_columns = [
            ...
            if column in top_display.columns
        ]

        if available_columns:
            display_top = top_display[available_columns].copy()
            ...
            rename_map = {
                ...
                "risk_level": "Risk",
                "dominant_sensor": "Affected Sensor"
            }
        ...
        st.info(
            "The compact top-anomalies summary is not available."
        )
