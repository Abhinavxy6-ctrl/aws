...
def to_india_time(values):
    """Convert timestamps to India Standard Time for dashboard display."""
...
if not live_weather.empty:
    ...
    current_update_time = datetime.now(ZoneInfo("Asia/Kolkata")).strftime(
        "%d %b %Y • %I:%M:%S %p"
    )
...
#
# IMPORTANT:
# The historical dashboard NEVER loads, creates, or processes the
# 46-million-row result CSV or the original historical dataset.
#
# All historical analytics below use the compact summary files
# generated previously from the full historical inference.
#
...
#
# No fallback historical processing is performed if a file is missing.
# ============================================================
...
HISTORICAL_SUMMARY_FILES = {
    "state": BASE_DIR / "skyguard_state_anomaly_summary.csv",
    "city": BASE_DIR / "skyguard_city_anomaly_summary.csv",
    "monthly": BASE_DIR / "skyguard_monthly_anomaly_summary.csv",
    "hourly": BASE_DIR / "skyguard_hourly_anomaly_summary.csv",
    "sensor": BASE_DIR / "skyguard_sensor_anomaly_summary.csv",
    "contributions": BASE_DIR / "skyguard_sensor_contributions.csv",
    "persistent_sensor": BASE_DIR / "skyguard_persistent_sensor_summary.csv",
    "top_anomalies": BASE_DIR / "skyguard_top_anomalies.csv",
}


@st.cache_data
def load_historical_summary_files():
    """Load only compact historical summary CSVs.

    This function intentionally performs NO historical inference and
    does NOT read the original 46M-row weather dataset.
    """

    loaded = {}

    for name, file_path in HISTORICAL_SUMMARY_FILES.items():
        if file_path.exists():
            try:
                ...
                loaded[name] = None
                st.warning(
                    f"Could not read historical summary "
...
historical_summary = load_historical_summary_files()

missing_historical_files = [
    path.name
    for name, path in HISTORICAL_SUMMARY_FILES.items()
    if historical_summary.get(name) is None
]

if missing_historical_files:
    st.warning(
        "Some compact historical summary files are unavailable. "
        "No historical re-processing will be attempted."
    )
    st.caption(
        "Missing/unreadable: " + ", ".join(missing_historical_files)
    )
...
historical_state = historical_summary.get("state")

if (
    historical_state is not None
    and not historical_state.empty
    and {"total_records", "anomalies"}.issubset(historical_state.columns)
):

    total_historical = int(
        pd.to_numeric(
            historical_state["total_records"],
            errors="coerce"
        ).fillna(0).sum()
    )
    ...
    historical_anomalies = int(
        pd.to_numeric(
            historical_state["anomalies"],
            errors="coerce"
        ).fillna(0).sum()
    )

    historical_normal = max(
        0,
        total_historical - historical_anomalies
    )
    ...
    with col4:
        st.metric(
            "Anomaly Rate",
            f"{historical_anomaly_rate:.2f}%"
        )

    st.caption(
        "Historical totals are read from the compact state summary; "
        "the original observation-level result CSV is not required."
    )
    ...
    historical_monthly = historical_summary.get("monthly")

    if (
        historical_monthly is not None
        and not historical_monthly.empty
        and {
            "month",
            "total_records",
            "anomalies",
            "anomaly_rate_percent"
        }.issubset(historical_monthly.columns)
    ):
        st.subheader("📈 Historical Monthly Anomaly Trend")

        monthly_plot = historical_monthly.copy()

        monthly_plot["month"] = pd.to_numeric(
            monthly_plot["month"],
            errors="coerce"
        )
        ...
        monthly_plot["Month"] = monthly_plot["month"].astype(int).map(
            lambda x: datetime(2000, x, 1).strftime("%b")
            if 1 <= x <= 12 else str(x)
        )
        ...
        fig_monthly = px.line(
            ...
            markers=True,
            title="Historical Monthly Anomaly Rate"
        )
    ...
    # --------------------------------------------------------
    # HOURLY HISTORICAL TREND
    # --------------------------------------------------------

    historical_hourly = historical_summary.get("hourly")

    if (
        historical_hourly is not None
        and not historical_hourly.empty
        and {
            "hour",
            "total_records",
            "anomalies",
            "anomaly_rate_percent"
        }.issubset(historical_hourly.columns)
    ):
        st.subheader("🕐 Historical Hourly Anomaly Pattern")

        hourly_plot = historical_hourly.copy()
        ...
        hourly_plot["Hour"] = (
            hourly_plot["hour"].astype(int).map(
                lambda x: f"{x:02d}:00"
            )
        )
        ...
        fig_hourly = px.line(
            ...
            y="anomaly_rate_percent",
            markers=True,
            title="Historical Hourly Anomaly Rate"
        )
    ...
    historical_sensor = historical_summary.get("sensor")
    historical_contributions = historical_summary.get("contributions")

    # SkyGuard historical AI is strictly aligned with the trained 3-feature model.
    # Only Temperature, Humidity, and Pressure are displayed here.
    HISTORICAL_FEATURES = ["temperature_C", "humidity_pct", "pressure_hPa"]
    HISTORICAL_SENSOR_LABELS = {
        "temperature_C": "Temperature",
        "humidity_pct": "Humidity",
        "pressure_hPa": "Pressure"
    }

    if historical_sensor is not None and not historical_sensor.empty:
        historical_sensor = historical_sensor.copy()
        if "sensor" in historical_sensor.columns:
            historical_sensor["sensor_key"] = historical_sensor["sensor"].astype(str).str.strip()
            historical_sensor["sensor_key"] = historical_sensor["sensor_key"].replace({
                "Temperature": "temperature_C",
                "Humidity": "humidity_pct",
                "Pressure": "pressure_hPa"
            })
            historical_sensor = historical_sensor[
                historical_sensor["sensor_key"].isin(HISTORICAL_FEATURES)
            ].copy()
            historical_sensor["sensor"] = historical_sensor["sensor_key"].map(HISTORICAL_SENSOR_LABELS)

    if historical_contributions is not None and not historical_contributions.empty:
        historical_contributions = historical_contributions.copy()
        if "feature" in historical_contributions.columns:
            historical_contributions["feature_key"] = historical_contributions["feature"].astype(str).str.strip()
            historical_contributions["feature_key"] = historical_contributions["feature_key"].replace({
                "Temperature": "temperature_C",
                "Humidity": "humidity_pct",
                "Pressure": "pressure_hPa"
            })
            historical_contributions = historical_contributions[
                historical_contributions["feature_key"].isin(HISTORICAL_FEATURES)
            ].copy()
            historical_contributions["feature"] = historical_contributions["feature_key"].map(HISTORICAL_SENSOR_LABELS)

    sensor_col1, sensor_col2 = st.columns(2)

    with sensor_col1:
        if (
            historical_sensor is not None
            and not historical_sensor.empty
            and {
                "sensor",
                "anomaly_count",
                "percentage"
            }.issubset(historical_sensor.columns)
        ):
            st.subheader("🌡️ Historical Sensor Anomalies")

            sensor_plot = historical_sensor.copy()
    ...
    with sensor_col2:
        if (
            historical_contributions is not None
            and not historical_contributions.empty
            and {
                "feature",
                "contribution_percent"
            }.issubset(historical_contributions.columns)
        ):
            st.subheader("📊 Sensor Error Contribution")

            contribution_plot = historical_contributions.copy()

            fig_contribution = px.pie(
                contribution_plot,
                names="feature",
                values="contribution_percent",
                title="Relative Reconstruction-Error Contribution"
            )
    ...
    st.info(
        "Historical validation uses precomputed SkyGuard 3-feature "
        "summaries (Temperature + Humidity + Pressure). "
        "No 46-million-row historical dataset is loaded or processed "
        "by this dashboard."
    )

else:
    st.warning(
        "Historical AI summary data is not available. "
        "The dashboard will not run historical re-processing."
    )
