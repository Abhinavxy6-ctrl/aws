...
HISTORICAL_3FEATURE_FILE = (
    BASE_DIR / "skyguard_3feature_historical_results.csv"
)
...
def generate_historical_3feature_results():
    ...
    if HISTORICAL_3FEATURE_FILE.exists():
    ...
    # --------------------------------------------------------
    # Process historical dataset in chunks
    # --------------------------------------------------------
    ...
    # Dataset contains approximately 46 million rows
    ...
    for chunk in pd.read_csv(
        ...
        scaled_values = realtime_scaler.transform(
            values
        )
        ...
        chunk_result = pd.DataFrame({
            ...
            "datetime": chunk["datetime"].values,

            "state": chunk["state"].values,

            "city": chunk["city"].values,
            ...
            "temperature_C":
                chunk["temperature_C"].values,

            "humidity_pct":
                chunk["humidity_pct"].values,

            "pressure_hPa":
                chunk["pressure_hPa"].values,
        ...
        status_text.text(
            f"Processing historical data: "
            f"{total_rows_processed:,} rows"
        )
