...
# Streamlit Cloud: set this to the direct URL of the ORIGINAL
# Indian_Weather_Dataset.csv. Do not point it at the old result file.
HISTORICAL_DATA_URL = ""
...
def generate_historical_ai_summary():
    """Run the same 3-feature SkyGuard inference directly on the source CSV.

    This intentionally does NOT build or save the 46M-row result CSV. It keeps
    only the dashboard aggregates and the 20 highest-scoring observations.
    """
    import urllib.request
    from collections import Counter

    source = HISTORICAL_DATA_URL.strip() if HISTORICAL_DATA_URL.strip() else HISTORICAL_DATA_FILE

    if isinstance(source, Path) and not source.exists():
        raise FileNotFoundError(
            "Historical source CSV was not found. On Streamlit Cloud, set "
            "HISTORICAL_DATA_URL to the direct URL of Indian_Weather_Dataset.csv."
        )
    ...
    total_records = 0
    anomaly_count = 0
    risk_counts = Counter({"NORMAL": 0, "WARNING": 0, "HIGH": 0, "CRITICAL": 0})
    top_rows = []
    total_rows_processed = 0

    if isinstance(source, str):
        # Download to a temporary file so pandas can stream it in chunks without
        # keeping the full 46M-row source dataset in RAM.
        import tempfile
    ...
    try:
        for chunk in pd.read_csv(
            ...
            if chunk.empty:
                progress = min(total_rows_processed / TOTAL_EXPECTED_ROWS, 1.0)
                progress_bar.progress(progress)
                status_text.text(f"Processing historical data: {total_rows_processed:,} rows")
                continue
            ...
            squared_errors = (input_tensor - reconstructed) ** 2
            anomaly_scores = torch.mean(squared_errors, dim=1).cpu().numpy()
            is_anomaly = anomaly_scores > realtime_threshold

            risk_level = np.where(
                anomaly_scores <= realtime_threshold,
                "NORMAL",
                np.where(
                    anomaly_scores <= realtime_threshold * 1.5,
                    "WARNING",
                    np.where(
                        anomaly_scores <= realtime_threshold * 2.5,
                        "HIGH",
                        "CRITICAL"
                    )
                )
            ...
            for level, count in zip(*np.unique(risk_level, return_counts=True)):
                risk_counts[str(level)] += int(count)

            # Keep only the best 20 rows, rather than retaining all 46M results.
            anomaly_indices = np.flatnonzero(is_anomaly)
            for idx in anomaly_indices:
                top_rows.append({
                    "datetime": chunk.iloc[idx]["datetime"],
                    "state": chunk.iloc[idx]["state"],
                    "city": chunk.iloc[idx]["city"],
                    "temperature_C": chunk.iloc[idx]["temperature_C"],
                    "humidity_pct": chunk.iloc[idx]["humidity_pct"],
                    "pressure_hPa": chunk.iloc[idx]["pressure_hPa"],
                    "anomaly_score": float(anomaly_scores[idx]),
                    "risk_level": str(risk_level[idx]),
                    "dominant_sensor": str(dominant_sensors[idx])
                })
            if len(top_rows) > 100:
                top_rows = sorted(top_rows, key=lambda x: x["anomaly_score"], reverse=True)[:20]
    ...
    top_anomalies = pd.DataFrame(
        sorted(top_rows, key=lambda x: x["anomaly_score"], reverse=True)[:20]
    )

    return {
        "total_records": int(total_records),
        "anomalies": int(anomaly_count),
        "normal": int(total_records - anomaly_count),
...
if total_historical > 0:
    ...
    historical_risk = pd.DataFrame({
        ...
        "Records": [
            historical_summary["risk_counts"].get("NORMAL", 0),
            historical_summary["risk_counts"].get("WARNING", 0),
            historical_summary["risk_counts"].get("HIGH", 0),
            historical_summary["risk_counts"].get("CRITICAL", 0)
        ]
    })
    ...
    st.plotly_chart(fig_historical_risk, width="stretch")

    # --------------------------------------------------------
    # TOP HISTORICAL ANOMALIES
    # --------------------------------------------------------
    st.subheader("🚨 Highest-Scoring Historical Observations")
    top_historical = historical_summary["top_anomalies"]
    if not top_historical.empty:
        st.dataframe(top_historical, width="stretch", hide_index=True)
    else:
    ...
    # --------------------------------------------------------
    st.info(
        "Historical validation uses the current 3-feature SkyGuard AI model: "
        "Temperature + Humidity + Pressure. Results are calculated directly "
        "from the original historical AWS source data; no precomputed result "
        "CSV is required."
    )
else:
    st.warning("Historical AI validation produced no results.")
