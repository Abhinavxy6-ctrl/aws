...
# ============================================================
# HISTORICAL AI VALIDATION — CALCULATE DIRECTLY FROM SOURCE DATA
# ============================================================
# This version does NOT create/download the historical result CSV.
# It reproduces the same 3-feature SkyGuard inference and keeps only
# the statistics and Top 20 anomalies required by the dashboard.
#
# IMPORTANT FOR STREAMLIT CLOUD:
# The ORIGINAL Indian_Weather_Dataset.csv must still be reachable.
# Put its direct Hugging Face /resolve/main/ URL in HISTORICAL_DATA_URL.
# ============================================================
...
# For Streamlit Cloud, set this to the direct-download URL of the
# ORIGINAL Indian_Weather_Dataset.csv. Leave empty for local use.
HISTORICAL_DATA_URL = ""

HISTORICAL_CHUNK_SIZE = 2_000_000
TOTAL_EXPECTED_ROWS = 46_082_160
...
def generate_historical_ai_summary():
    """Run the historical 3-feature SkyGuard model directly.

    No 46-million-row result dataframe is retained and no historical
    result CSV is written. Only counters and the Top 20 anomalies are kept.
    """

    source = (
        HISTORICAL_DATA_URL.strip()
        if HISTORICAL_DATA_URL.strip()
        else HISTORICAL_DATA_FILE
    )

    if isinstance(source, Path) and not source.exists():
        st.error(
            "Historical source dataset was not found. For Streamlit Cloud, "
            "upload the ORIGINAL Indian_Weather_Dataset.csv to Hugging Face "
            "and set HISTORICAL_DATA_URL to its direct /resolve/main/ URL."
        )
        return {
            "total_records": 0, "anomalies": 0, "normal": 0,
            "risk_counts": {"NORMAL": 0, "WARNING": 0, "HIGH": 0, "CRITICAL": 0},
    ...
    try:
        reader = pd.read_csv(
            source,
            usecols=HISTORICAL_SOURCE_COLUMNS,
            chunksize=HISTORICAL_CHUNK_SIZE,
            low_memory=False,
        )

        for chunk in reader:
            chunk = chunk.dropna(subset=HISTORICAL_FEATURE_COLUMNS).copy()
            if chunk.empty:
                continue

            values = chunk[HISTORICAL_FEATURE_COLUMNS].astype("float32").values
            scaled_values = realtime_scaler.transform(values)
            input_tensor = torch.tensor(scaled_values, dtype=torch.float32)
            ...
            squared_errors = (input_tensor - reconstructed) ** 2
            anomaly_scores = (
                torch.mean(squared_errors, dim=1).cpu().numpy()
            )

            is_anomaly = anomaly_scores > realtime_threshold

            risk_level = np.where(
                anomaly_scores <= realtime_threshold, "NORMAL",
                np.where(
                    anomaly_scores <= realtime_threshold * 1.5, "WARNING",
                    np.where(
                        anomaly_scores <= realtime_threshold * 2.5, "HIGH",
                        "CRITICAL"
                    )
                )
            )

            feature_errors = squared_errors.cpu().numpy()
            dominant_indices = np.argmax(feature_errors, axis=1)
            sensor_names = np.array(["Temperature", "Humidity", "Pressure"])
            dominant_sensors = sensor_names[dominant_indices]

            total_records += len(chunk)
            anomaly_count += int(is_anomaly.sum())
            for level in risk_counts:
                risk_counts[level] += int(np.sum(risk_level == level))

            anomaly_indices = np.flatnonzero(is_anomaly)
            if len(anomaly_indices):
                candidates = pd.DataFrame({
                    "datetime": chunk["datetime"].iloc[anomaly_indices].values,
                    "state": chunk["state"].iloc[anomaly_indices].values,
                    "city": chunk["city"].iloc[anomaly_indices].values,
                    "lat": chunk["lat"].iloc[anomaly_indices].values,
                    "lon": chunk["lon"].iloc[anomaly_indices].values,
                    "temperature_C": chunk["temperature_C"].iloc[anomaly_indices].values,
                    "humidity_pct": chunk["humidity_pct"].iloc[anomaly_indices].values,
                    "pressure_hPa": chunk["pressure_hPa"].iloc[anomaly_indices].values,
                    "anomaly_score": anomaly_scores[anomaly_indices],
                    "is_anomaly": True,
                    "risk_level": risk_level[anomaly_indices],
                    "dominant_sensor": dominant_sensors[anomaly_indices],
                })
                top_candidates = pd.concat([top_candidates, candidates], ignore_index=True)
                top_candidates = top_candidates.nlargest(20, "anomaly_score").reset_index(drop=True)

            progress_bar.progress(min(total_records / TOTAL_EXPECTED_ROWS, 1.0))
            status_text.text(f"Running historical SkyGuard AI: {total_records:,} records processed")

        return {
            "total_records": total_records,
            "anomalies": anomaly_count,
            "normal": total_records - anomaly_count,
            "risk_counts": risk_counts,
            "top_anomalies": top_candidates,
        }

    except Exception as e:
        st.error(f"Unable to process the historical source dataset. Error: {e}")
        return {
            "total_records": 0, "anomalies": 0, "normal": 0,
            "risk_counts": {"NORMAL": 0, "WARNING": 0, "HIGH": 0, "CRITICAL": 0},
            "top_anomalies": pd.DataFrame(),
...
with st.spinner(
    "Running current 3-feature SkyGuard AI on historical AWS data..."
):
    historical_summary = generate_historical_ai_summary()

historical_ai_results = historical_summary["top_anomalies"]
...
with st.spinner(
    "Running current 3-feature SkyGuard AI "
    "on historical AWS data..."
):

    historical_ai_results = (
        generate_historical_3feature_results()
    )
...
if not historical_ai_results.empty:
    ...
    total_historical = historical_summary["total_records"]
    historical_anomalies = historical_summary["anomalies"]
    historical_normal = historical_summary["normal"]
    ...
    st.subheader(
        "Historical AI Risk Distribution"
    )

    historical_risk = (
        pd.Series(historical_summary["risk_counts"])
        .reindex(["NORMAL", "WARNING", "HIGH", "CRITICAL"], fill_value=0)
        .rename("Records")
        .rename_axis("Risk Level")
        .reset_index()
    )

    historical_risk.columns = [
        "Risk Level",
        "Records"
    ]
    ...
    if not top_historical.empty:
        ...
        st.success(
            "No historical anomalies were detected."
        )
    ...
    st.info(
        "Historical validation uses the current "
        "3-feature SkyGuard AI model: "
    ...
    st.warning(
        "Historical AI validation produced no results."
    )
