...
# Historical results are stored as 23 GitHub-friendly CSV parts.
# All 23 parts together contain the same records as the original
# skyguard_3feature_historical_results.csv file.
HISTORICAL_FILES = [
    BASE_DIR / f"skyguard_history_{i:02d}.csv"
    for i in range(1, 24)
]


@st.cache_data
def generate_historical_3feature_results():
    result_columns = [
        "datetime",
        "state",
        "city",
    ...
    # Load all 23 parts in their original order.  No rows are removed,
    # re-scored, aggregated, or otherwise changed here.
    missing_files = [
        str(path.name)
    ...
    for index, historical_file in enumerate(HISTORICAL_FILES, start=1):
        status_text.text(
            f"Loading historical data file {index}/23: "
            f"{historical_file.name}"
        )

        chunk = pd.read_csv(historical_file, low_memory=False)
        results.append(chunk)
        progress_bar.progress(index / len(HISTORICAL_FILES))
