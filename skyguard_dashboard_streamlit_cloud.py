...
if detail_mode == "stations_analyzed":
    ...
    if valid_results.empty:
        ...
        st.dataframe(station_view, width="stretch", hide_index=True)
    ...
    if detail.empty:
        ...
        st.dataframe(normal_view, width="stretch", hide_index=True)
    ...
    if detail.empty:
        ...
        st.dataframe(critical_view, width="stretch", hide_index=True)
    ...
    if frozen_detail.empty:
        ...
        st.dataframe(frozen_view, width="stretch", hide_index=True)
        st.warning(
...
# ============================================================
# HISTORICAL 3-FEATURE DATA FROM HUGGING FACE
# ============================================================

# The large historical results file is stored outside GitHub on the
# Hugging Face Hub.  This keeps the Streamlit/GitHub repository small.
HISTORICAL_3FEATURE_URL = (
    "https://huggingface.co/datasets/Abhinavbwn123/skyguard/"
    "resolve/main/skyguard_3feature_historical_results.csv"
)


@st.cache_data(show_spinner=False)
def load_historical_3feature_results():
    """
    Load the pre-computed 3-feature historical SkyGuard results from
    Hugging Face.

    The CSV already contains anomaly scores and classifications, so
    Streamlit does NOT re-run the autoencoder over the original
    46-million-row source dataset.
    """
    ...
    try:
        ...
        # Keep the dataframe reasonably memory-efficient on Streamlit Cloud.
        for column in [
            "temperature_C",
            "humidity_pct",
            "pressure_hPa",
        ...
        if "is_anomaly" in historical.columns:
            if historical["is_anomaly"].dtype == object:
                historical["is_anomaly"] = (
                    historical["is_anomaly"]
                    .astype(str)
                    .str.strip()
                    .str.lower()
                ...
                historical["is_anomaly"] = historical["is_anomaly"].astype(bool)

        # These repeated strings can otherwise consume a lot of RAM.
        for column in ["state", "city", "risk_level", "dominant_sensor"]:
            if column in historical.columns:
                historical[column] = historical[column].astype("category")

        return historical

    except Exception as e:
        st.error(
            "Unable to load the historical SkyGuard results from Hugging Face. "
            f"Error: {e}"
        )
...
with st.spinner(
    "Loading pre-computed 3-feature SkyGuard historical results from Hugging Face..."
):
    historical_ai_results = load_historical_3feature_results()
